 ...
 * @version V1.3.1
 */
public class EditDeliverAddressActivity extends BaseActivity implements OnClickListener,OnWheelScrollListener,OnFocusChangeListener
{
    private static final String TAG = EditDeliverAddressActivity.class.getSimpleName();
    ...
    private class StateReceiver extends BroadcastReceiver
    {
        ...
        public void onReceive(Context context, Intent intent)
        {
            ...
            if (action != null
                    && action.equals(Const.ACTION_EDIT_DELIVER_ADDR_SUCCESS))
            {
                mTitleBar.onBackBtnClick();
                return;
            }
            if (action != null
                    && action.equals(Const.ACTION_DEL_DELIVER_ADDR_SUCCESS))
            {
                mTitleBar.onBackBtnClick();
                return;
            }
            if (action != null
                    && action.equals(Const.ACTION_SET_DEFALUT_DELIVER_ADDR_SUCCESS))
            {
                mTitleBar.onBackBtnClick();
                return;
            }
            if (action != null
                    && action.equals(Const.ACTION_EDIT_DELIVER_ADDR_FAILED))
            {
                Toast.makeText(context, R.string.edit_deliver_address_txt, Toast.LENGTH_LONG).show();
                return;
            }
        }
    }
...
//        Logger.d(TAG, "onCreate()[access]");
        setContentView(R.layout.edit_deliver_address);
        ...
        mTitleBar.setEditBtnText(R.string.edit_deliver_address_txt);
        ...
        id = bundle.getString(Const.EXTRA_SELECTED_DELIVER_ADDRESS_ID);
        position = bundle.getInt(Const.EXTRA_SELECTED_DELIVER_ADDRESS_POS);

        mReceivers = AccountManager.getInstance().getCurrentAccount()
                .getReceivers();
        mReceiver = mReceivers.get(position);
        
        Logger.d(TAG, "mReceiver="+mReceiver);
        mDeliverName.setText(mReceiver.getName());
        mDetialAddress.setText(mReceiver.getAddress());
        mPhoneNumber.setText(mReceiver.getMobile());
        mPostCode.setText(mReceiver.getZipCode());
        mLocationArea.setText(mReceiver.getAreaStore().getDisplayName());
    }
    ...
    public void onClick(View v)
    {
        ...
        switch (v.getId())
        {
            case R.id.edit_btn:
                ...
                if (mSelectArea == null)
                {
                    receiver.setAreaStore(mReceiver.getAreaStore());
                }
                
                AccountManager.getInstance().editDeliverAddress(receiver);
                break;
            case R.id.location_area_text:
                ...
                break;
            case R.id.delete_deliver_address:
                AccountManager.getInstance().delDeliverAddress(mReceiver);
                
                break;
            case R.id.set_default_deliver_address:
                AccountManager.getInstance().setDefaultDeliverAddress(mReceiver);
                break;
            default:
                break;
        }
...
//        wheel.setCyclic(true);
    ...
    private void regReceiver()
    {
        if(mStateReceiver == null)
        {
            ...
            filter.addAction(Const.ACTION_EDIT_DELIVER_ADDR_FAILED);
            filter.addAction(Const.ACTION_EDIT_DELIVER_ADDR_SUCCESS);
            filter.addAction(Const.ACTION_DEL_DELIVER_ADDR_SUCCESS);
            filter.addAction(Const.ACTION_DEL_DELIVER_ADDR_FAILED);
            filter.addAction(Const.ACTION_SET_DEFALUT_DELIVER_ADDR_SUCCESS);
            filter.addAction(Const.ACTION_SET_DEFALUT_DELIVER_ADDR_FAILED);
        ...
        public synchronized void handleMessage(final android.os.Message msg)
        {
            synchronized (mTitleBar)
            {
                ...
                Animation animation = AnimationUtils.loadAnimation(
                        EditDeliverAddressActivity.this, animResId);
